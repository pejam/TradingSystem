            ...
            TransferOwnershipPlaceRequest request)
        {
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    // Set the content type and SOAPAction header
                    client.DefaultRequestHeaders.Add("Content-Type", "text/xml");
                    client.DefaultRequestHeaders.Add("SOAPAction", "http://internalTradeServices/TransferOwnershipPlace_SI");
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        // Read and deserialize the response content
                        string responseXml = await response.Content.ReadAsStringAsync();
                        ApiResult<TransferOwnershipPlaceResult> apiResult = DeserializeResponse<TransferOwnershipPlaceResult>(responseXml);
